...
- haze-electron-app
  ...
  - Bezpośrednio zależy od haze-runtime.
  - Pośrednio zależy od haze-keystone-ui (tymczasowo, może zostać usunięte później?)
- haze-keystone-ui
...
- haze-runtime-api
  - To jest współdzielony kod i API dla środowiska wykonawczego haze. Może być używany dla aplikacji renderującej (strona przeglądarki)
...
Jeśli jesteś zainteresowany konkretną logiką strony, możesz przejść do `haze-keystone-ui/src/windows/main/views`. Pliki `.vue` w tym folderze są głównymi komponentami używanymi w launcherze. Prefiks pliku to domena interfejsu użytkownika.
...
Jeśli jesteś zainteresowany podstawową logiką, możesz przejść do `haze-runtime/services/`. Każdy plik w tym miejscu reprezentuje usługę dla konkretnej domeny/aspektu logiki launchera. W trakcie tego procesu powinieneś również zwrócić uwagę na odpowiadające im pliki w `haze-runtime-api/services/`, które deklarują interfejs rzeczywistych usług.

Kilka przykładów:

1. `haze-runtime/services/InstanceService.ts` zawiera implementację API do dodawania/usuwania/aktualizacji instancji. `haze-runtime-api/services/InstanceService.ts` zawiera interfejs `InstanceService`
2. `haze-runtime/services/InstanceVersionService.ts` zawiera implementację API do sprawdzania stanu wersji instancji. Określi, której wersji będzie używać instancja i czy powinniśmy zainstalować tę wersję.
3. `haze-runtime/services/InstallService.ts` zawiera implementację API do instalacji Minecraft/Forge/Fabric itp.
4. `haze-runtime/services/LaunchService.ts` zawiera implementację API do uruchamiania instancji.
...
Powinieneś ustawić `CURSEFORGE_API_KEY` tworząc plik `.env` w `haze-electron-app`. Ten plik `.env` jest dodany do pliku `.gitignore`.
...
Chyba że kod w `haze-keystone-ui` uległ zmianie, nie musisz budować tego ponownie.
