...
- haze-electron-app
  - 用 Electron 实现运行时。
  - 它依赖 haze-runtime。
  - 它的部分内容依赖 haze-keystone-ui（临时的，可能将在未来被删除）
- haze-keystone-ui
  - 启动器的默认 UI。
...
- haze-runtime-api
  - 这是 haze 运行时的共享代码与 API。它可用于渲染器应用程序（浏览器侧）
...
如果你对特定的页面逻辑感兴趣，你可以看看 `haze-keyston-ui/src/windows/main/views`。这个文件夹下的 `.vue` 文件是启动器中使用的主要组件。文件的前缀是用户界面的关键字。
...
如果你对核心逻辑感兴趣，你可以看看 `haze-runtime/services/`。它下面的每个文件都将一个特定领域/业务的逻辑封装成了“服务”。在阅读服务的过程中，你也需要看看 `haze-runtime-api/services/` 下的对应的服务声明。

一些例子：

1. `haze-runtime/services/InstanceService.ts`包含添加/删除/更新实例的 API 实现。`haze-runtime-api/services/InstanceService.ts`包含了`InstanceService`的接口。
2. `haze-runtime/services/InstanceVersionService.ts`包含检查实例版本健康状况的 API 实现。它将确定实例将使用什么版本，以及我们是否应该安装该版本。
3. `haze-runtime/services/InstallService.ts`包含安装 Minecraft/Forge/Fabric 等的API实现。
4. `haze-runtime/services/LaunchService.ts`包含启动实例的 API 实现。
...
#### 设置环境变量
在`haze-electron-app`下创建`.env`文件来设置`CURSEFORGE_API_KEY`。`.env`文件已被添加到`.gitignore`文件中。
...
除非你在 `haze-keystone-ui` 有新的改动，你已经不需再重新跑这个命令了。
