...
- haze-electron-app
  ...
  - Це безпосередньо залежить від haze-runtime.
  - Неявно залежить від haze-keystone-ui (тимчасово, може бути вилучено пізніше?)
- haze-keystone-ui
...
- haze-runtime-api
  - Це спільний код та API для виконання haze. Він може бути використаний для програми рендерингу (на стороні браузера)
...
Якщо вас цікавить конкретна логіка сторінок, ви можете перейти до `haze-keystone-ui/src/windows/main/views`. Файли `.vue` у цій теці є основним компонентом, що використовується у програмі запуску. Префікс файлу - це домен інтерфейсу користувача.
...
Якщо вас цікавить логіка роботи ядра, ви можете перейти до каталогу `haze-runtime/services/`. Кожен файл у цьому каталозі представляє службу для певного домену/аспекту логіки запуску. Під час цього процесу вам також слід знати про відповідні файли у каталозі `haze-runtime-api/services/`, які описують інтерфейс відповідних сервісів.

Наведемо кілька прикладів:

1. Файл `haze-runtime/services/InstanceService.ts` містить реалізацію API для додавання/видалення/оновлення екземплярів. Файл `haze-runtime-api/services/InstanceService.ts` містить інтерфейс `InstanceService`.
2. Файл `haze-runtime-api/services/InstanceVersionService.ts` містить реалізацію API для перевірки стану версії екземпляра. Вона визначає, яку версію буде використовувати екземпляр, і чи потрібно встановлювати цю версію.
3. Файл `haze-runtime/services/InstallService.ts` містить реалізацію API для встановлення Minecraft/Forge/Fabric тощо.
3. `haze-runtime/services/LaunchService.ts` містить реалізацію API для запуску екземпляра.
...
Вам слід встановити `CURSEFORGE_API_KEY`, створивши файл `.env` у каталозі `haze-electron-app`. Цей файл `.env` буде додано до файлу `.gitignore`.
...
Якщо код під `haze-keystone-ui` не змінився, вам не потрібно збирати його знову.
