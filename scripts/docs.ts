...
const renderLink = (fileName: string, line: number) => `<a href="https://github.com/FaaizJohar/HazeClient/blob/master/${fileName}#L${line}" target="_blank" rel="noreferrer">${fileName}:${line}</a>`
...
async function generateDocs() {
  ...
  project.traverse((refl) => {
    if (refl.kindOf(ReflectionKind.Module) && refl instanceof ContainerReflection) {
      ...
      const simpleName = refl.name.startsWith('@haze') ? refl.name.substring(6) : refl.name
