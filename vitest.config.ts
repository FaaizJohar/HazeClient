...
// `@/` is the renderer's path alias (haze-keystone-ui/src). Map it so unit
// tests can import composables/utils that use it. The regex form only matches
// `@/...` and leaves scoped packages like `@haze/...` untouched.
const keystoneSrc = join(__dirname, 'haze-keystone-ui/src').replace(/\\/g, '/')
