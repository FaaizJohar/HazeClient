...
- haze-electron-app
  ...
  - This directly depends on the haze-runtime.
  - This implicitly depends on haze-keystone-ui (temporally, might be removed later?)
- haze-keystone-ui
...
- haze-runtime-api
  - This is the shared code & API for Haze Client runtime. It can be used for renderer app (browser side)
...
If you are interested in a specific page logic, you can go to `haze-keystone-ui/src/windows/main/views`. The `.vue` files under this folder are the major component used in the launcher. The prefix of the file are the domain of the UI.
...
If you are interested in core logic, you can goto `haze-runtime/services/`. Each file under it are representing a service for a specific domain/aspect of the launcher logic. During this process, you should also aware about the corresponding files under the `haze-runtime-api/services/`, which declare the interface of the actual services.

Some examples:

1. `haze-runtime/services/InstanceService.ts` contains the API implementation of add/remove/update of instances. The `haze-runtime-api/services/InstanceService.ts` contains the interface of the `InstanceService`
2. `haze-runtime/services/InstanceVersionService.ts` contains the API implementation of checking instance version health. It will determine what version will the instance use, and whether should we install that version.
3. `haze-runtime/services/InstallService.ts` contains the API implementation of install Minecraft/Forge/Fabric and etc.
3. `haze-runtime/services/LaunchService.ts` contains the API implementation of launch an instance.
...
You should set the `CURSEFORGE_API_KEY` by creating a `.env` file under `haze-electron-app`. This `.env` file is added in `.gitignore` file.
...
Unless the code under `haze-keystone-ui` changed, you don't need to build this again.
