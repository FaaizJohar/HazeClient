...
import { GameOptionsState, InstanceInstallStatus, InstanceModsGroupState, InstanceState, JavaState, LocalVersions, ModpackState, PeerState, ResourceState, Saves, ServerInfoState, ServiceChannels, ServiceKey, Settings, SharedState, StateMetadata, UserState } from '@haze/runtime-api'
...
function createServiceChannels(): ServiceChannels {
  ...
  ipcRenderer.on('commit-batch', (_, id, commits: { type: string; payload: any }[]) => {
    const state = states[id]?.deref()
    if (state) {
      for (const { type, payload } of commits) {
        (state as any)[kEmitter].emit(type, payload);
        (state as any)[kEmitter].emit('*', type, payload)
        const methods = (state as any)[kMethods]
        const method = methods.find(([name]: [string]) => name === type)
      ...
      pendingCommits[id].push(...commits)
    }
  })
