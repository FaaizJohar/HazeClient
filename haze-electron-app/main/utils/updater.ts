...
} from '@haze/file-transfer'
import { Tracker, onDownloadSingle } from '@haze/installer'
import {
  DownloadUpdateTrackerEvents,
  ElectronUpdateOperation,
  ReleaseInfo,
} from '@haze/runtime-api'
import { DownloadUpdateOptions, LauncherAppUpdater } from '@haze/runtime/app'
import { AnyError, isSystemError } from '@haze/utils'
...
async function downloadAsarUpdate(
  ...
  const urls = gfw.inside
    ? [`https://github.com/FaaizJohar/HazeClient/releases/download/v${version}/${file}`]
    : [`https://github.com/FaaizJohar/HazeClient/releases/download/v${version}/${file}`]
...
async function hintUserDownload(): Promise<void> {
  shell.openExternal('https://github.com/FaaizJohar/HazeClient/releases')
}
...
async function downloadAppInstaller(
  ...
  const destination = join(app.getPath('downloads'), 'HazeClient.appinstaller')
  const url = `https://github.com/FaaizJohar/HazeClient/releases/latest/download/hazeclient.appinstaller`
      ...
      Object.assign(executor, {
        ...
        createRequest: (options: any, callback: any) => {
          if (gfw.inside) {
            ...
            options.pathname = `/Soft_Mirrors/github-release/FaaizJohar/HazeClient/LatestRelease/${basename(options.pathname)}`
...
export class ElectronUpdater implements LauncherAppUpdater {
  ...
  async #getUpdateFromSelfHost(): Promise<ReleaseInfo> {
    ...
    const response = await this.app
      .fetch(`https://api.github.com/repos/FaaizJohar/HazeClient/releases/latest`, {
        headers: {
