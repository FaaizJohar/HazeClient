...
async function buildMain(options: BuildOptions, slient = false) {
  ...
  await copy(path.join(__dirname, '../haze-ui/dist'), path.join(__dirname, './dist/renderer'))
  
  // Copy haze-in-game mod to dist so it is bundled
  if (existsSync(path.join(__dirname, '../haze-in-game/build/libs/haze-client-core-1.0.0.jar'))) {
    await copyFile(
      path.join(__dirname, '../haze-in-game/build/libs/haze-client-core-1.0.0.jar'), 
      path.join(__dirname, './dist/haze-in-game.jar')
    )
    if (!slient) console.log(`Copied haze-in-game.jar`)
  } else {
    if (!slient) console.warn(`haze-client-core-1.0.0.jar not found! It will not be bundled.`)
  }
...
async function start() {
  ...
  const config: Configuration = {
    ...
    async afterPack(context) {
      ...
      if (!existsSync(src)) {
        src = join(context.appOutDir, 'Haze Client.app/Contents/Resources/app.asar')
    ...
    async artifactBuildCompleted(context) {
      ...
      if (context.target && context.target.name === 'appx') {
        await buildAppInstaller(version, path.join(__dirname, './build/output/haze.appinstaller'), electronBuilderConfig.appx!.publisher!)
      }
    },
  }
