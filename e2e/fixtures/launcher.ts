 ...
 *  - Per-test temp directory for both Electron `userData` and the haze game
...
const ELECTRON_ENTRY = resolve(REPO_ROOT, 'haze-electron-app/dist/index.js')
...
const ELECTRON_BIN = process.platform === 'win32'
  ? resolve(REPO_ROOT, 'haze-electron-app/node_modules/electron/dist/electron.exe')
  : process.platform === 'darwin'
    ? resolve(REPO_ROOT, 'haze-electron-app/node_modules/electron/dist/Electron.app/Contents/MacOS/Electron')
    : resolve(REPO_ROOT, 'haze-electron-app/node_modules/electron/dist/electron')
    ...
    while (Date.now() < deadline) {
      ...
      try {
        ...
        for (const w of mainViewed) {
          seen.add(`main-id-${w.id}:url-${w.url}:loading-${w.loading}`)
        }
      } catch (err) {
        seen.add(`evaluate-error:${(err as Error).message}`)
      }
      // If we see at least one window in main process, let's try to return app.firstWindow()
      try {
...
export const test = base.extend<{
  ...
  launcher: async ({ launcherOptions }, use, testInfo) => {
    ...
    const tempRoot = await mkdtemp(join(tmpdir(), 'haze-e2e-'))
    ...
    // Layout matches what haze-runtime/app/LauncherApp.ts expects:
    //   {appDataPath}/haze/root  -> contains the game data path
    ...
    if (!launcherOptions.bootstrap) {
      const xmclDir = join(appDataPath, 'haze')
    ...
    const app = await _electron.launch({
      ...
      args: [ELECTRON_ENTRY],
      cwd: resolve(REPO_ROOT, 'haze-electron-app/dist'),
      env: {
        ...process.env,
