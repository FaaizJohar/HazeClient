import { CurseforgeV1Client } from '@haze/curseforge'
import {
  InstanceLockSchema,
...
import { ModrinthV2Client } from '@haze/modrinth'
import { isValidCurseforgeRef, isValidModrinthRef, ResourceManager, ResourceMetadata, UpdateResourcePayload } from '@haze/resource'
import {
  ModpackException,
  ModpackServiceKey,
  ModpackState,
...
export class ModpackService extends AbstractService implements IModpackService {
  ...
  async exportModpack(options: ExportModpackOptions) {
    ...
    curseforgeZip?.addBuffer(xmclJson, 'haze.json')
    modrinthZip?.addBuffer(xmclJson, 'haze.json')
    offlineZip?.addBuffer(xmclJson, 'haze.json')
    ...
    const instanceLockContent: InstanceLockSchema | undefined = await readJson(
      ...
      .catch(() => undefined)
    const lockHashLookup = Object.fromEntries(
  ...
  async openModpack(modpackFile: string): Promise<SharedState<ModpackState>> {
    ...
    return store.registerOrGet(`modpack-file://${modpackFile}`, async () => {
      ...
      if (zip.entries['haze.json']) {
        try {
          const b = await readEntry(zip.file, zip.entries['haze.json'])
