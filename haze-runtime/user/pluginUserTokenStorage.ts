import { AUTHORITY_DEV, AUTHORITY_MICROSOFT, AUTHORITY_MOJANG } from '@haze/runtime-api'
...
export const pluginUserTokenStorage: LauncherAppPlugin = (app) => {
  ...
  const storage: UserTokenStorage = {
    put: async (user, token) => {
      cache[`haze/${getStorageKey(user.authority)}/${user.id}`] = token
      await app.secretStorage.put(`haze/${getStorageKey(user.authority)}`, user.id, token)
    },
    get: async (user) => {
      const cached = cache[`haze/${getStorageKey(user.authority)}/${user.id}`]
      if (cached) return cached
      const token = await app.secretStorage.get(`haze/${getStorageKey(user.authority)}`, user.id)
      if (token) {
