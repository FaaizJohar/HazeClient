import { AUTHORITY_MICROSOFT } from '@haze/runtime-api'
import { MicrosoftAuthenticator, MojangClient } from '@haze/user'
...
export const pluginOfficialUserApi: LauncherAppPlugin = async (app) => {
  ...
  const system = new MicrosoftAccountSystem(logger,
    ...
    new MicrosoftOAuthClient(
      ...
      async (directRedirectToLauncher) => {
        ...
        return (directRedirectToLauncher ? `http://localhost:${port}/auth` : `https://haze.app/auth?port=${port}`)
      },
      (response) => {
  ...
  app.protocol.registerHandler('haze', ({ request, response }) => {
