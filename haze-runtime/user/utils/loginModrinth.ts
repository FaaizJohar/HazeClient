...
export async function loginModrinth(app: LauncherApp, userService: UserService, scopes: string[], invalidate: boolean, signal?: AbortSignal) {
  ...
  if (!token) {
    ...
    // Always use api.haze.app — the GFW-region Azure Function endpoint
    // (haze-highfreq-function.azurewebsites.net/api/modrinth-auth) is
    // currently returning a broken response (content encoding mismatch),
    // and api.haze.app works regardless of region.
    const authUrl = new URL('https://api.haze.app/modrinth/auth')
