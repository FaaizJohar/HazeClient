...
import { SharedState, createPromiseSignal } from '@haze/runtime-api'
...
export class ServiceStateContainer<T = any> implements ServiceStateContext {
  ...
  track(client: Client) {
    let batchedCommits: { type: string; payload: any }[] = []
    let timeoutId: any = null

    const handler = (type: any, payload: any) => {
      batchedCommits.push({ type, payload })
      
      if (!timeoutId) {
        // Game active mode throttle (10s) vs normal debounce (16ms)
        const delay = (global as any).isGameActiveMode ? 10000 : 16
        
        timeoutId = setTimeout(() => {
          if (batchedCommits.length > 0) {
            client.send('commit-batch', this.id, batchedCommits)
            batchedCommits = []
          }
          timeoutId = null
        }, delay)
