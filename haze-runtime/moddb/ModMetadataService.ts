import { isNotNull } from '@haze/core/utils'
import { download } from '@haze/file-transfer'
import { onDownloadSingle, Tracker } from '@haze/installer'
import { isValidModrinthId, ResourceDomain, ResourceManager, type Resource } from '@haze/resource'
import { kResourceManager } from '~/resource'
import { ModMetadataServiceKey, type ModMetadataService as IModMetadataService, type ModMetadata, DownloadModMetadataDbTask, DownloadModMetadataDbTrackerEvents } from '@haze/runtime-api'
...
import { jsonObjectFrom, SqliteWASMDialect } from '@haze/sqlite'
...
export class ModMetadataService extends AbstractService implements IModMetadataService {
  ...
  async #ensureDb() {
    ...
    const sha1 = await (await this.app.fetch('https://haze.blob.core.windows.net/releases/db.sqlite.sha1')).text()
    const dbPath = this.getAppDataPath('db.sqlite')
    const actual = await checksumFromStream(createReadStream(dbPath), 'sha1').catch(() => '')
    if (actual !== sha1) {
      const url = 'https://haze.blob.core.windows.net/releases/db.sqlite'
