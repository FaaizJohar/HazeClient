import { ProjectMappingService as IProjectMappingService, ProjectMapping, ProjectMappingServiceKey, Settings } from '@haze/runtime-api'
...
import { SqliteWASMDialect } from '@haze/sqlite'
...
export class ProjectMappingService extends AbstractService implements IProjectMappingService {
  ...
  private async ensureDatabase(init = false) {
    ...
    await this.mutex.of('project-mapping').runExclusive(async () => {
      let original = `https://haze.blob.core.windows.net/project-mapping/${locale}.sqlite`
      ...
      if (!hasLocaleDb) {
        original = 'https://haze.blob.core.windows.net/project-mapping/en.sqlite'
