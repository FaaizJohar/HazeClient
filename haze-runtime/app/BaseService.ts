...
export class BaseService extends AbstractService implements IBaseService {
  ...
  constructor(
    ...
    super(app, async () => {
        ...
        const electronNet = (() => { try { return require('electron').net } catch { return null } })()
        if (electronNet && electronNet.isOnline) {
          this.isOffline = !(await electronNet.isOnline())
      }
      this.checkUpdate()
    })
  }
  ...
  async makeDesktopShortcut() {
    ...
    if (process.platform === 'win32') {
      const shortcutPath = join(desktopDir, 'Haze Client.lnk')
