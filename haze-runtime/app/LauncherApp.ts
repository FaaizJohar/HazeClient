import { getPlatform } from '@haze/core'
import { InstalledAppManifest, Platform, createPromiseSignal } from '@haze/runtime-api'
...
export class LauncherApp extends EventEmitter {
  ...
  get userAgent() {
    ...
    return `voxelum/x_minecraft_launcher/${version} (haze.app)`
  }
  ...
  readonly server: Server = createServer((req, res) => {
    this.protocol.handle({
      method: req.method,
      url: new URL(req.url ?? '/', 'haze://launcher'),
      headers: req.headers,
      body: req,
    }).then((resp) => {
  ...
  /**
   * Determine the root of the project. By default, it's %APPDATA%/haze
   */
  protected async setup() {
    ...
    // register haze protocol
    if (!this.host.isDefaultProtocolClient('haze')) {
      const result = this.host.setAsDefaultProtocolClient('haze')
      if (result) {
        this.logger.log('Successfully register the haze protocol')
      } else {
        this.logger.log('Fail to register the haze protocol')
      }
    }
  ...
  protected async getStartupUrl(): Promise<string | undefined> {
    if (!IS_DEV && process.platform === 'win32') {
      ...
      if (process.argv.length > 1) {
        ...
        const protocolOption = process.argv.find(a => a.startsWith('haze://'))
        if (protocolOption) {
        ...
        this.logger.log('Didn\'t find haze:// protocol')
      }
    }
