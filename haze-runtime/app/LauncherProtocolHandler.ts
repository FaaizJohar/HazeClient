 ...
 *
 * The drop in request or localhost request will be transformed into haze:// protocol
 */
  ...
  async handle(request: RequestOptions): Promise<ResolvedResponse> {
    ...
    const context: Context = {
      request: {
        ...
        url: typeof request.url === 'string' ? new URL(request.url, 'haze://launcher') : request.url,
