 ...
 *
 * Run with:
 *   BMCL_PROBE=1 npx vitest run haze-runtime/network/BmclDownload.e2e.test.ts
 *
...
import type { DownloadResult } from '@haze/file-transfer'
...
run('BMCLAPI adaptive download e2e', () => {
  it(
    ...
    async () => {
      ...
      const dir = await mkdtemp(join(tmpdir(), 'haze-bmcl-'))
      try {
