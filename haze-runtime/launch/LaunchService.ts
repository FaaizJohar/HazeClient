import { MinecraftFolder, type LaunchOption as ResolvedLaunchOptions, type ResolvedVersion, type ServerOptions, type ResolvedServerVersion, createMinecraftProcessWatcher, generateArguments, generateArgumentsServer, launch, launchServer } from '@haze/core'
import { AUTHORITY_DEV, type CreateLaunchShortcutOptions, type GameProcess, type LaunchService as ILaunchService, LAUNCH_FAILURE_PREFIX, LaunchException, type LaunchOptions, LaunchServiceKey, type ReportOperationPayload } from '@haze/runtime-api'
import { offline } from '@haze/user'
...
import { isSystemError } from '@haze/utils'
import { VersionService } from './VersionService'
import { LauncherApp } from '../app/LauncherApp'
import { UTF8 } from '../util/encoding'
import type { LaunchMiddleware } from './LaunchMiddleware'
import { ensureDir } from '@haze/installer/utils'
...
export class LaunchService extends AbstractService implements ILaunchService {
  ...
  async #generateOptions(options: LaunchOptions, version: ResolvedVersion, accessToken?: string) {
    ...
    const launcherName = `Haze Client (${this.app.version})`
    ...
    const hazeCorePath = join(this.app.appDataPath, 'haze-core-framework.jar')
    if (existsSync(hazeCorePath)) {
      launchOptions.extraJVMArgs = launchOptions.extraJVMArgs || []
      launchOptions.extraJVMArgs.push(`-javaagent:${hazeCorePath}`)
    }

    // Haze Client In-Game Mod Injection
    const hazeInGamePath = join(this.app.appDataPath, 'haze-in-game.jar')
    const bundledInGamePath = join(__dirname, 'haze-in-game.jar')
    try {
      if (existsSync(bundledInGamePath)) {
        // Always copy to ensure latest version is injected
        await require('fs/promises').copyFile(bundledInGamePath, hazeInGamePath)
      }
    } catch (e) {
      this.error(new Error(`Failed to copy bundled haze-in-game.jar: ${e}`))
    }

    if (existsSync(hazeInGamePath)) {
      launchOptions.extraJVMArgs = launchOptions.extraJVMArgs || []
      launchOptions.extraJVMArgs.push(`-Dfabric.addMods=${hazeInGamePath}`)
      this.log(`Injected Haze In-Game Mod from ${hazeInGamePath}`)
    }

    // Haze Client Native FPS Boost: Vector SIMD
    launchOptions.extraJVMArgs = launchOptions.extraJVMArgs || []
    launchOptions.extraJVMArgs.push('--add-modules=jdk.incubator.vector')

    if (this.ipcPort > 0) {
      launchOptions.extraJVMArgs = launchOptions.extraJVMArgs || []
      launchOptions.extraJVMArgs.push(`-Dhazeclient.ipc.port=${this.ipcPort}`)
    }
  ...
  async launch(options: LaunchOptions) {
    ...
    if (!this.ipcServer) {
      this.ipcServer = createServer((socket) => {
        this.log('IPC Client connected from ' + socket.remoteAddress)
        this.ipcClients.add(socket)
        
        // Send a welcome payload
        socket.write(JSON.stringify({ type: 'hello', version: this.app.version }) + '\n')
        
        socket.on('data', (data) => {
          this.log(`IPC Data received: ${data.toString()}`)
        })
        ...
        socket.on('error', (err) => {
          this.error(new Error(`IPC Client error: ${err.message}`))
        })
      })
      ...
      await new Promise<void>((resolve) => {
        this.ipcServer!.listen(0, '127.0.0.1', () => {
          ...
          if (address && typeof address !== 'string') {
            ...
            this.log(`Haze IPC Server listening on port ${this.ipcPort}`)
          }
          resolve()
        })
      })
    ...
    try {
      ...
      if ('inheritances' in version) {
        ...
        if (global.process.platform === 'win32' || require('os').platform() === 'win32') {
          // FPS Killer Fix 4: GPU Selection
          try {
            const { stdout } = await promisify(require('child_process').exec)('wmic path win32_VideoController get Name')
            if (stdout.toLowerCase().includes('nvidia')) {
              op.extraExecOption = op.extraExecOption || {}
              op.extraExecOption.env = op.extraExecOption.env || { ...global.process.env }
              op.extraExecOption.env!.DXVK_FILTER_DEVICE_NAME = 'NVIDIA'
              this.log('NVIDIA GPU detected. Forcing dGPU selection via DXVK_FILTER_DEVICE_NAME.')
            }
          } catch (e) {
            this.warn('Failed to detect NVIDIA GPU:', e)
          }

          // FPS Killer Fix 5: RTSS Warning
          try {
            const { stdout } = await promisify(require('child_process').exec)('tasklist')
            if (stdout.toLowerCase().includes('rtsshooksloader64.exe') || stdout.toLowerCase().includes('rtss.exe')) {
              this.warn('RTSS or RivaTuner detected. This is incompatible with Sodium/OptiFine.')
              // We just log warning here. The UI dialog is handled by PreLaunchScanService.
              op.extraJVMArgs = op.extraJVMArgs || []
              op.extraJVMArgs.push('-Dsodium.checks.win32.rtss=false')
            }
          } catch (e) {
            this.warn('Failed to check for RTSS:', e)
          }
        }
      ...
      // Haze Client Native FPS Boost: Process Priority Elevation
      if (process.pid) {
        if (global.process.platform === 'win32' || require('os').platform() === 'win32') {
          try {
            spawn('wmic', ['process', 'where', `processid="${process.pid}"`, 'CALL', 'setpriority', '128'], {
              windowsHide: true,
            }).on('error', (e) => this.warn(`Failed to set high priority for PID ${process.pid}: ${e}`))
            this.log(`Elevated process ${process.pid} to High Priority for maximum FPS.`)
            // Drop launcher priority to BelowNormal (16384)
            spawn('wmic', ['process', 'where', `processid="${global.process.pid}"`, 'CALL', 'setpriority', '16384'], {
              windowsHide: true,
            }).on('error', (e) => this.warn(`Failed to set below normal priority for launcher PID ${global.process.pid}: ${e}`))
          } catch (e) {
            this.warn(`Failed to set high priority for PID ${process.pid}: ${e}`)
          }
        }
      }
      ...
      // Enable Game Active Mode Throttle
      (global as any).isGameActiveMode = true
      ...
      watcher.on('error', (err) => {
        ...
        // Missing Feature 2: Playtime Tracker
        try {
          const playtimePath = join(options.gameDirectory, 'playtime.json')
          let playtimeData = []
          if (existsSync(playtimePath)) {
            const data = require('fs').readFileSync(playtimePath, 'utf8')
            playtimeData = JSON.parse(data)
          }
          playtimeData.push({
            instanceId: options.gameDirectory,
            startTime,
            durationMs: playTime
          })
          require('fs').writeFileSync(playtimePath, JSON.stringify(playtimeData, null, 2), 'utf8')
        } catch (e) {
          this.warn(`Failed to write playtime data: ${e}`)
        }

        // Restore launcher priority to Normal (32)
        if (global.process.platform === 'win32' || require('os').platform() === 'win32') {
          try {
            spawn('wmic', ['process', 'where', `processid="${global.process.pid}"`, 'CALL', 'setpriority', '32'], {
              windowsHide: true,
            })
          } catch (e) {}
        }
        
        // Disable Game Active Mode Throttle
        (global as any).isGameActiveMode = false
        ...
        Promise.all(errPromises).catch((e) => { this.error(e) }).finally(async () => {
          ...
          let gcPauseCount: number | undefined
          try {
            const logsDir = join(options.gameDirectory, 'logs')
            if (existsSync(logsDir)) {
              const { readdir, readFile } = require('fs/promises')
              const files = await readdir(logsDir)
              const sparkLogName = files.find((f: string) => f.startsWith('spark-') && f.endsWith('.log'))
              if (sparkLogName) {
                const content = await readFile(join(logsDir, sparkLogName), 'utf-8')
                const fpsMatch = content.match(/Average FPS:\s*(\d+)/i)
                const gcMatch = content.match(/GC pause count:\s*(\d+)/i)
                if (fpsMatch) averageFps = parseInt(fpsMatch[1], 10)
                if (gcMatch) gcPauseCount = parseInt(gcMatch[1], 10)
              }
            }
          } catch (e) {
            // ignore
   ...
   * written into the instance's `logs/` folder using a `haze-abnormal-exit-`
  ...
  async #persistAbnormalExitLog(gameDirectory: string, info: {
    ...
    const header = [
      `# Haze Client abnormal-exit dump`,
