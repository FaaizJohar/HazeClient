import { AUTHORITY_DEV } from '@haze/runtime-api'
import { YggdrasilTexture, YggdrasilTexturesInfo } from '@haze/user'
...
export const pluginYggdrasilHandler: LauncherAppPlugin = (app) => {
  ...
  app.protocol.registerHandler('haze', async ({ request, response, handle }) => {
    if (request.url.pathname.startsWith('/yggdrasil')) {
      ...
      if (pathname === '/' || pathname === '') {
        ...
        response.body = JSON.stringify({
          meta: {
            implementationName: 'haze-offline-server',
            implementationVersion: '0.0.1',
            serverName: 'Haze Client Offline Server',
          },
          skinDomains: [
            'localhost',
          ],
