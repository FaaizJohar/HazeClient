  ...
  >
    ...
    >
      <div class="v-card-title screenshot-card__title cursor-move">
        <v-icon size="18" color="white" class="opacity-80">perm_media</v-icon>
        <span>Media Center</span>
        <v-chip
          v-if="mounted && (display.length > 0)"
          size="x-small"
          variant="tonal"
          color="white"
          class="ml-1"
      ...
      <v-btn
        v-shared-tooltip="'Open Media Center'"
        variant="text"
        icon
        size="small"
        color="white"
        class="no-drag"
        @click="showMediaCenter"
      >
...
const { show: showMediaGalleryDialog } = useDialog<{ instancePath: string }>('media-gallery')

const showMediaCenter = () => {
  showMediaGalleryDialog({ instancePath: props.instance.path })
}
