  ...
  <SettingCard title="Java" icon="coffee">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-4 px-2">
      ...
      <div>
        ...
        <!-- FPS Killer Fix 2: Performance Presets -->
        <div class="font-weight-medium mb-2 flex items-center text-body-2 mt-4">
          <v-icon start size="small" color="primary">speed</v-icon>
          Performance Presets
        </div>
        <div class="flex gap-2 mb-3">
          <v-btn size="small" variant="tonal" @click="applyPreset('minimal')" v-shared-tooltip="'Minimal basic GC flags'">Minimal</v-btn>
          <v-btn size="small" variant="tonal" @click="applyPreset('balanced')" v-shared-tooltip="'Balanced performance and memory'">Balanced</v-btn>
          <v-btn size="small" variant="tonal" color="primary" @click="applyPreset('aikar')" v-shared-tooltip="'Aikar\'s Flags for optimal latency (G1GC)'">Aikar's Flags</v-btn>
        </div>
...
import { BaseServiceKey, JavaRecord, JavaServiceKey } from '@haze/runtime-api'
...
function applyPreset(preset: 'minimal' | 'balanced' | 'aikar') {
  let flags = ''
  if (preset === 'minimal') {
    flags = '-XX:+UseG1GC'
  } else if (preset === 'balanced') {
    flags = '-XX:+UseG1GC -XX:MaxGCPauseMillis=200 -XX:+UnlockExperimentalVMOptions -XX:+DisableExplicitGC'
  } else if (preset === 'aikar') {
    flags = '-XX:+UseG1GC -XX:+ParallelRefProcEnabled -XX:MaxGCPauseMillis=200 -XX:+UnlockExperimentalVMOptions -XX:+DisableExplicitGC -XX:+AlwaysPreTouch -XX:G1NewSizePercent=30 -XX:G1MaxNewSizePercent=40 -XX:G1HeapRegionSize=8M -XX:G1ReservePercent=20 -XX:G1HeapWastePercent=5 -XX:G1MixedGCCountTarget=4 -XX:InitiatingHeapOccupancyPercent=15 -XX:G1MixedGCLiveThresholdPercent=90 -XX:G1RSetUpdatingPauseTimePercent=5 -SurvivoRatio=32 -XX:+PerfDisableSharedMem -XX:MaxTenuringThreshold=1'
  }
  
  if (!vmOptions.value) {
    vmOptions.value = flags
  } else {
    // Append but avoid duplicates (simplified)
    const current = vmOptions.value
    if (!current.includes('-XX:+UseG1GC')) {
      vmOptions.value = `${current} ${flags}`
    } else {
      vmOptions.value = flags // Replace if they already have some flags
    }
  }
}
</script>
