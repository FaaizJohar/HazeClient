...
import { InstanceModsServiceKey, InstanceResourcePacksServiceKey, InstanceSavesServiceKey, InstanceShaderPacksServiceKey, ModpackServiceKey } from '@haze/runtime-api'
...
async function getVersionDetail(version: StoreProjectVersion): Promise<StoreProjectVersionDetail> {
  if (upstream.value?.type === 'modrinth-modpack') {
    ...
    try {
      ...
      const { FileRelationType } = await import('@haze/curseforge')
