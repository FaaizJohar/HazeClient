<template>
  ...
  <div
    v-else
    class="flex h-full w-full flex-col items-center justify-center setup-unloaded text-white select-none"
  >
    <div class="setup-splash-container">
      <img
        class="setup-splash-logo"
        src="http://launcher/icons/logoDark"
        alt="Haze Client Logo"
      />
      <h1 class="setup-splash-title">Haze Client</h1>
      <p class="setup-splash-subtitle">By Cavrix Core Technologies</p>
      
      <div class="setup-splash-loader"></div>
    </div>
  </div>
</template>
...
import { BaseServiceKey, Drive, InvalidDirectoryErrorCode } from '@haze/runtime-api'
...
.setup-splash-container {
  display: flex;
  flex-direction: column;
  align-items: center;
  opacity: 0;
  transform: scale(0.97);
  animation: splashEnter 1.2s cubic-bezier(0.16, 1, 0.3, 1) forwards 0.2s;
}
...
.setup-splash-loader::after {
  ...
  background: #ffffff;
  border-radius: 2px;
  animation: indeterminateProgress 1.5s cubic-bezier(0.4, 0, 0.2, 1) infinite;
}

@keyframes fadeIn {
  from { background-color: #000000; }
  to { background-color: #0d0d0d; }
}
