<template>
  <v-app v-if="!showSetup" class="h-full max-h-screen overflow-hidden" :class="{ 'dark': isDark }">
    ...
    >
      <HazeSideBar />
      <main class="relative flex max-h-full flex-1 flex-col overflow-auto m-3 rounded-2xl bg-black/40 shadow-2xl backdrop-blur-md" :class="mainClasses">
...
import HazeDynamicBackground from '@/views/HazeDynamicBackground.vue'
...
import HazeSideBar from '@/views/HazeSideBar.vue'
...
onErrorCaptured((err, instance, info) => {
  ...
  if (state.value) {
    // Silently log to telemetry instead of cascading
    // Assume telemetry or logger is available, if not just prevent cascade
  }
