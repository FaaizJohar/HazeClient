import type { Instance } from '@haze/instance'
import type { GameOptionsState, JavaRecord, UserProfile } from '@haze/runtime-api'
import {
...
// Keep this list in sync with `haze-ui/src/windows/main/router.ts`.
    ...
    async execute(args) {
      ...
      if (kind === 'root') {
        const entries: VfsEntry[] = [
          ...
          { type: 'dir', name: 'launch-failures', description: 'launcher-captured abnormal-exit dumps (haze-abnormal-exit-*)' },
...
const BASE_RULES = `You are the haze (Haze Client) assistant. You help the user manage their Minecraft launcher: switching instances, installing or toggling mods / resourcepacks / shaders, launching the game, and diagnosing crashes from logs.
