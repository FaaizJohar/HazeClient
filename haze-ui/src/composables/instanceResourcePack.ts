...
import { PackMeta } from "@haze/resourcepack";
import {
  GameOptions,
...
export const kInstanceResourcePacks: InjectionKey<
  ReturnType<typeof useInstanceResourcePacks>
> = Symbol("InstanceResourcePacks");

function getResourcepackFormat(meta: any) {
  return meta ? meta.format ?? meta.pack_format : 3;
}
function getResourcePackItem(
  resource: Resource,
  enabled: Set<string>
): InstanceResourcePack {
  const p: InstanceResourcePack = {
    path: resource.path,
    fileName: resource.fileName,
    version: "",
    enabled:
      enabled.has(resource.fileName) ||
      enabled.has(`file/${resource.fileName}`),
    name: resource.name,
    id: `file/${
      resource.fileName.endsWith(".zip")
        ? resource.fileName
        : resource.fileName + ".zip"
    }`,
    pack_format: 0,
    ...
    acceptingRange:
      packFormatVersionRange[
        getResourcepackFormat(resource.metadata.resourcepack)
      ] ?? "[*]",
...
export function useInstanceResourcePacks(
  path: Ref<string>,
  gameOptions: Ref<GameOptions | undefined>
) {
  const { watch, install } = useService(InstanceResourcePacksServiceKey);
  const { state, isValidating, revalidate, error } = useState(
  ...
  function getResourcePackItemFromGameSettingName(
    ...
    if (resourcePackName !== "vanilla") {
      ...
      pack.id = resourcePackName.startsWith("file")
        ? resourcePackName
        : `file/${resourcePackName}`;
    } else {
  ...
  const result = computed(() => {
    const enabledArray = [
      ...(gameOptions.value?.resourcePacks ?? []),
    ].reverse();
  ...
  function enable(pack: (InstanceResourcePack | string)[]) {
    const newEnabled = [
      ...pack.map((e) => (typeof e === "string" ? e : e.id)),
      ...enabled.value.map((e) => e.id),
    ];
    install({
      path: path.value,
