...
import { Instance } from "@haze/instance";
...
const props = defineProps<{
  ...
  /** Optional pretty name from the servers.dat entry. Falls back to host. */
  serverName?: string;
  /** Skip the small refresh icon (useful in compact layouts). */
  hideRefresh?: boolean;
}>();
...
const displayName = computed(() => props.serverName ?? props.instance?.server?.name ?? props.instance?.server?.host ?? "");
const protocol = useMinecraftProtocol(computed(() => props.instance?.runtime.minecraft));

const { status, pinging, refresh, refreshIfStale } = useServerStatus(server, protocol);
...
const copyAddress = () => {
  ...
  navigator.clipboard
    ...
    .catch((err) => {
      ...
      notify({
        level: "error",
        title: t('shared.copyFailed'),
        body: t('shared.copyFailedHint'),
      });
    });
};
</script>
