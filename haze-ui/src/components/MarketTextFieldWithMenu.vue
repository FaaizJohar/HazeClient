      ...
      >
        ...
        <v-tabs-window v-model="tab" class="overflow-auto flex min-h-0 flex-grow">
          <v-tabs-window-item :value="0" class="tab">
            ...
            >
              <v-chip v-if="mounted" key="__all__" filter variant="outlined" label>
                {{ t('minecraftVersion.all') }}
              </v-chip>
              <v-chip v-if="mounted" v-for="v of versionIds" :key="v" filter variant="outlined" label>
                {{ v }}
              </v-chip>
            </v-chip-group>
...
import { ModsSearchSortField } from '@haze/curseforge'
