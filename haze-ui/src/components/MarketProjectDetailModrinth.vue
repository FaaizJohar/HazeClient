...
import { ProjectMapping, ProjectMappingServiceKey } from '@haze/runtime-api'
...
const dependencies = computed(() => {
  ...
  return (
    deps.value.map(({ recommendedVersion, versions, project, type, parent }) => {
      ...
      const { progress, total } = useModrinthTask(
        taskManagerTasks,
...
const curseforgeId = computed(
  () =>
    props.curseforge ||
    props.allFiles.find((v) => v.modrinth?.projectId === props.projectId && v.curseforge)
  ...
  >
    <div>
      <v-btn
        ...
        variant="text"
      >
        <v-icon start>haze:curseforge</v-icon>
        Curseforge
      </v-btn>
    </div>
  </Hint>
