...
Перейдіть до репозиторію github, https://github.com/Voxelum/haze-client-template, натисніть Use this template, щоб створити репозиторій лаунчера
...
Потім відкрийте haze для завантаження вашого середовища розробки,
Знайдіть іконку haze в системному треї, клацніть правою кнопкою миші та виберіть `Переглянути інтерфейс стороннього лаунчера`
...
A:
Перш ніж відповісти на це питання, вам потрібно розуміти механізм екземплярів (instance) haze, це не складно, деталі дивіться в [цій документації](https://haze.app/zh/faq))
...
```ts
import { LaunchServiceKey } from '@haze/runtime-api'
...
```ts
import { InstallServiceKey } from '@haze/runtime-api'
