...
describe('parseCli — globals + dispatch', () => {
  ...
  test('--root with value', () => {
    const r = parseCli(['--root', '/data/haze', 'launch', '/inst/foo'], makeRegistry())
    expect(r.globals.root).toBe('/data/haze')
  })
  ...
  test('skips electron-noise leading args until a command name is recognised', () => {
    const r = parseCli(['/usr/bin/haze', '--gpu-blacklist=', 'launch', '/inst/foo'], makeRegistry())
...
describe('formatHelp / formatCommandHelp', () => {
  test('top-level help mentions command', () => {
    const txt = formatHelp(makeRegistry(), { programName: 'haze' })
    expect(txt).toContain('haze')
  ...
  test('command help shows positionals + flags', () => {
    ...
    const txt = formatCommandHelp(cmd, { programName: 'haze' })
